...
result_path = file_path/'result'/'jupyter.txt'
...
def exploit(ip_, index):
    try:
        ...
        if 'Password' in res.text or '密码' in res.text or res.status_code != 200 or 'submit' in res.text:
            return
...
threads = []
with open(r'D:\Downloads\tmp\jupyter.txt', 'r') as file:
